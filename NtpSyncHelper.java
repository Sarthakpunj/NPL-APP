...
public class NtpSyncHelper {
    ...
    public static String syncNtp(String ntpServerName) {
        StringBuilder result = new StringBuilder();

        bias = 0; // Replace with the desired bias

        result.append("Synchronizing NTD:-\n");
        for (String host : NTD_IP) {
            result.append(host).append("\n");
        }

        try {
            loop(ntpServerName, result);
        } catch (Exception e) {
            e.printStackTrace();
            result.append("Alert: Unable to get NTP Time...\n");
            try {
            ...
            syncNtp(ntpServerName);
        }

        return result.toString();
    }

    private static void loop(String ntpServerName, StringBuilder result) throws Exception {
        while (true) {
            try {
                openNtdCount = 0;
